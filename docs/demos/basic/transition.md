...
gui.add(config, 'enabled').name('启用');
gui.add(config, 'rotation').name('旋转');
gui.add(config, 'effect', { 淡入淡出: 'fade', 黑场: 'black', 白场: 'white' }).name('效果');

const speed = gui.addFolder('速度/持续时间');
speed.add(config, 'speed_mode', { 持续时间: 'duration', 速度: 'speed' }).name('模式');
