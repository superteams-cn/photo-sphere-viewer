...
export function callViewer(log: string): Cypress.Chainable<Viewer> {
  cy.log(`Viewer: ${log}`);
  return cy.window(NO_LOG).its('viewer', NO_LOG);
}
...
export function checkPosition(position: SphericalPosition) {
  callViewer('check position').then((viewer) => expect(viewer.getPosition()).to.deep.eq(position));
}

export function checkZoom(zoom: number) {
  callViewer('check zoom').then((viewer) => expect(viewer.getZoomLevel()).to.eq(zoom));
}

export function checkPanorama(name: string) {
  callViewer('check panorama').then((viewer) => expect(viewer.config.panorama as string).to.eq(BASE_URL + name));
}

export function setPanorama(name: string) {
  callViewer(`set panorama "${name}"`).then((viewer) => viewer.setPanorama(BASE_URL + name, { transition: false }));
  waitViewerReady();
}
...
export function checkEventHandler(handler: Cypress.Agent<sinon.SinonStub>, params: any) {
  cy.wrap(handler, NO_LOG).should('have.been.calledWithMatch', params);
}
